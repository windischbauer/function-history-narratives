...
export class SubstringService {
  ...
  longestCommonSubstringInArray(arr: string[]): string {
    let longestCommon = null;

    for (let i = 0; i < arr.length - 1; i++) {
      for (let j = i; j < arr.length; j++) {
        const part = this.longestCommonSubstring(arr[i], arr[j]);
        if (longestCommon === null) {
          longestCommon = part;
        }

        if (part.length < longestCommon.length) {
          longestCommon = part;
        }
      }
    }
